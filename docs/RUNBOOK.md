...
| ComfyUI process dies silently mid-render (log just stops, no traceback) | ZLUDA/driver hard-crash class; often the same cudnn/VAE issue escalating. Correlates with RAM pressure on the 16 GB box (concurrent downloads 2026-08-04; full studio stack resident 2026-08-08) and with ComfyUI-Manager registry fetches running during renders | Check user/comfyui.log for the last op; ensure the cudnn-off node is present. Manager is set network_mode=offline (user/__manager/config.ini, 2026-08-08) - keep it that way; flip to public only when intentionally updating nodes. Avoid heavy downloads during renders. If persistent, drop sampler to euler and re-test |
